...
protected:
    ...
    void dropEvent(QDropEvent *event) override {
        if (event->mimeData()->hasText()) {
            ...
            // Verify we are dropping the CORRECT item back to its place?
            // User didn't strictly say we must check type match, but it makes sense.
            // If I drop "Towel" on "Sheet" shelf, should it work?
            // "Prevent students from taking too many" -> Put back to inventory.
            // Let's assume strict matching for realism.
            if (text == m_itemName) {
